...
from src.agents.prompts.quality import QUALITY_ANALYST_SYSTEM
...
async def analyze_quality(state: AgentState) -> dict[str, Any]:
    """LangGraph node: run the code-quality analyst agent."""
    evidence: dict[str, Any] = state["evidence_package"]["quality"]
    logger.info(
        "quality_analyst started endpoints=%d calls=%d",
        len(evidence.get("endpoints", [])),
        len(evidence.get("frontend_calls", [])),
    )
    ...
    try:
        ...
        logger.exception("quality_analyst_failed")
        issues = []

    logger.info("quality_analyst done issues=%d", len(issues))
